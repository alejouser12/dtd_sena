...
// ── Redirigir según rol ──────────────────────────────────────────
if (esAprendiz()) {
    redirect_to('mod/aprendiz_index.php');   // ← CORREGIDO
}
if (esInstructor()) {
    redirect_to('mod/instructor_dashboard.php');
}

// Admin: mostrar el index normal
require_once 'conexion/AlertaDAO.php';
$alertaDAO    = new AlertaDAO();
$conteo       = $alertaDAO->obtenerConteoAlertas();
$totalAlertas = $conteo['total'] ?? 0;
$alertasAltas = $conteo['altas'] ?? 0;
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    .alerta-badge { position:absolute; top:-8px; right:-8px; background:linear-gradient(135deg,#dc2626,#ef4444); color:white; font-size:12px; font-weight:700; min-width:20px; height:20px; border-radius:10px; display:flex; align-items:center; justify-content:center; padding:0 6px; box-shadow:0 2px 5px rgba(0,0,0,.2); z-index:10; border:2px solid white; }
    .dark-mode .alerta-badge { border-color:#2d2d2d; }
    .alerta-tooltip { position:absolute; bottom:10px; left:10px; font-size:11px; background:rgba(0,0,0,.05); padding:2px 8px; border-radius:12px; color:#666; }
    </style>
</head>
<body>
    <div id="loader">
        ...
        <ul class="cards-grid">
            ...
            <li class="card" id="alertas-card">
                <a href="<?= htmlspecialchars(app_url('mod/alertas.php')) ?>">
                    ...
                    <?php if ($totalAlertas > 0): ?><div class="alerta-badge"><?= $totalAlertas ?></div><?php endif; ?>
                    <?php if ($alertasAltas > 0): ?><div class="alerta-tooltip">🔴 <?= $alertasAltas ?> alta(s)</div><?php endif; ?>
                </a>
            </li>
            <li class="card">
